...
int16_t final_left = LINE_FOLLOW_SPEED_RUN;
int16_t final_right = LINE_FOLLOW_SPEED_RUN;

/* 按 IR_Read() 的定义：sensor=1 表示该路检测到黑线。 */
left_hit = (uint8_t)((g_ir_data.sensor[0] != 0U) || (g_ir_data.sensor[1] != 0U));
center_hit = (uint8_t)(g_ir_data.sensor[2] != 0U);
right_hit = (uint8_t)((g_ir_data.sensor[3] != 0U) || (g_ir_data.sensor[4] != 0U));
left_score = (uint8_t)((g_ir_data.sensor[0] != 0U ? 2U : 0U) + (g_ir_data.sensor[1] != 0U ? 1U : 0U));
right_score = (uint8_t)((g_ir_data.sensor[3] != 0U ? 1U : 0U) + (g_ir_data.sensor[4] != 0U ? 2U : 0U));
score_diff = (left_score > right_score) ? (uint8_t)(left_score - right_score) : (uint8_t)(right_score - left_score);
turn_outer_speed = LINE_FOLLOW_SPEED_CURVE_OUTER;
if ((!center_hit) || (score_diff >= 2U))
{
turn_outer_speed = LINE_FOLLOW_SPEED_EDGE_OUTER;
}

/* 目标只有一个：尽量让中间一路单独压线；转弯时允许外侧轮提速，换更大的左右差速。 */
if (center_hit && (left_score == 0U) && (right_score == 0U))
{
...
{
final_left = turn_outer_speed;
final_right = LINE_FOLLOW_SPEED_TURN;
g_line_follow_last_dir = -1;
g_line_debug_mode = "LEFT_CORR";
}
else if (right_score > left_score)
{
final_left = LINE_FOLLOW_SPEED_TURN;
final_right = turn_outer_speed;
g_line_follow_last_dir = 1;
g_line_debug_mode = "RIGHT_CORR";
}
else if (center_hit && (left_hit || right_hit))
{
/* 中间仍在压线，但左右分值打平时，沿用上一拍方向继续压弯。 */
if (g_line_follow_last_dir < 0)
{
final_left = LINE_FOLLOW_SPEED_CURVE_OUTER;
final_right = LINE_FOLLOW_SPEED_TURN;
g_line_debug_mode = "LEFT_KEEP";
}
else if (g_line_follow_last_dir > 0)
{
final_left = LINE_FOLLOW_SPEED_TURN;
final_right = LINE_FOLLOW_SPEED_CURVE_OUTER;
g_line_debug_mode = "RIGHT_KEEP";
}
else
{
final_left = LINE_FOLLOW_SPEED_WIDE;
final_right = LINE_FOLLOW_SPEED_WIDE;
g_line_debug_mode = "CENTER_WIDE";
}
}
else
{
/* 全白丢线时按上一拍方向缓慢搜线，仍然不超过 50% 占空比。 */
if (g_line_follow_last_dir < 0)
{
final_left = LINE_FOLLOW_SPEED_SEARCH;
final_right = LINE_FOLLOW_SPEED_TURN;
g_line_debug_mode = "LEFT_SEARCH";
}
else if (g_line_follow_last_dir > 0)
{
final_left = LINE_FOLLOW_SPEED_TURN;
final_right = LINE_FOLLOW_SPEED_SEARCH;
g_line_debug_mode = "RIGHT_SEARCH";
}
else
{
final_left = LINE_FOLLOW_SPEED_WIDE;
...
final_left = LineFollow_ClampPWM(final_left);
final_right = LineFollow_ClampPWM(final_right);
...
/* 直接对应物理左右侧：PA8/PA10 为左侧，PA9/PA11 为右侧。 */
